...
namespace Portakal
{
	bool MeshLoader::LoadMesh(const String& path, MeshLoadResult& resultOut)
	{
		//Get importer
	...
	bool MeshLoader::LoadMeshFromMemory(const MemoryView& memory, MeshLoadResult& resultOut)
	{
		//Get importer
		Assimp::Importer importer;

		//Read
		const aiScene* pScene = importer.ReadFileFromMemory(memory.GetMemory(),memory.GetSize(),
			aiProcess_CalcTangentSpace |
			aiProcess_Triangulate |
			aiProcess_JoinIdenticalVertices |
			aiProcess_GenNormals |
