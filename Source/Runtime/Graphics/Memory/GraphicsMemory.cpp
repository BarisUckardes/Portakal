...
namespace Portakal
{
    ...
    UInt64 GraphicsMemory::Allocate(const UInt64 sizeInBytes)
    {
        ...
        for (UInt32 i = 0; i < mBlocks.GetSize(); i++)
        {
            ...
            const UInt64 sizeLeft = block.SizeInBytes - sizeInBytes;
    ...
    void GraphicsMemory::Free(const UInt64 offsetInBytes)
    {
    ...
    Portakal::GraphicsMemory::CompactReport GraphicsMemory::GetCompactReport(const UInt32 index)
    {
        //Check min
        UInt32 min = index;
